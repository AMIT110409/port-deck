...
const STACK_GROUPS = [
  ...
  },
  {
    ...
    color: "#3DDC97",
    items: ["LangChain · LangGraph", "RAG / FAISS / Qdrant", "Claude & GPT APIs", "Prompt engineering", "Fine-tuning (LoRA/QLoRA)", "LLMOps & evaluation"],
  },
  {
    label: "Cloud & Infrastructure",
    color: "#00B4D8",
    items: ["Azure · AWS · GCP", "Terraform · Pulumi", "Ansible playbooks", "Vault secrets mgmt", "Kong API Gateway", "CI/CD (GitHub Actions / ADO)"],
  },
  {
    label: "Observability & Data",
    color: "#E8A33D",
    items: ["Prometheus · Grafana", "Loki · Jaeger", "Celonis · Power BI", "Databricks", "Kafka · RabbitMQ", "PostgreSQL · Redis"],
  },
  {
...
const PROJECTS = [
  ...
  },
  {
    ...
    points: [
      ...
      "Horizontal pod autoscaling handles peak order volumes (10x burst capacity)",
      "Full audit trail via structured logging to Azure Log Analytics",
    ],
    stack: ["Claude API", "PaddleOCR", "Celonis", "SAP SOAP/BAPI", "Azure AKS", "Docker", "FastAPI"],
    status: "Live in UAT",
    category: "ai",
  },
  {
    ...
    summary:
      "FAISS-backed retrieval-augmented Q&A over regulated pharma documentation, deployed as a secure, air-gapped Kubernetes cluster with strict RBAC and audit logging.",
    points: [
      "Air-gapped K8s cluster with OPA policies enforcing data residency rules",
      "Custom network policies — no egress to public internet from compliance pods",
      "Reduced manual query resolution time by 60% across pilot users",
    ],
    stack: ["RAG", "FAISS", "LangChain", "FastAPI", "Kubernetes", "OPA", "Vault"],
    status: "Delivered",
    category: "ai",
  },
  {
  ...
  },
  {
    ...
    summary:
      "Reusable enterprise agent platform — the shared backbone behind multiple client AI deployments — packaged as a multi-tenant Kubernetes deployment with per-tenant namespace isolation.",
    points: [
    ...
    stack: ["LangGraph", "Python", "Azure AKS", "Helm", "Vault", "Redis", "PostgreSQL"],
  ...
  },
  {
    ...
    summary:
      "Real-time supply chain monitoring platform with a Neo4j graph backend, deployed on AWS EKS with Kafka for event streaming and Prometheus for SLO tracking.",
    points: [
      "Kafka consumer group of 8 pods processing 50k+ events/min with autoscaling",
      "Neo4j Causal Cluster deployed on EKS with custom persistent volume configuration",
      "Full observability: Prometheus, Grafana, Loki, and Jaeger traces in one Helm release",
    ],
    stack: ["Neo4j", "Kafka", "Python", "AWS EKS", "Docker", "Prometheus", "Grafana"],
    status: "Delivered",
    category: "infra",
  },
];
